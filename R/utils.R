...
##     df <- list(Y=d[[1]], X=d[[2]] - cutoff, orig.cutoff=cutoff,
##                var.names=names(d)[1:2])
##     df$sigma2 <- d$sigma2
##     if(is.null(d$weights)) d$weights <- rep(1L, length(df$X))
##     df$w <- d$weights
##     df$len <- length(X)
##     df$pos <- which.max(df$X>=0)
...
##     X <- d[[3]] - cutoff
##     df <- list(Y=cbind(d[[1]], d[[2]]), X=X, orig.cutoff=cutoff,
