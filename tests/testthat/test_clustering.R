test_that("Test clustering formulas", {
    ...
    set.seed(42)
    clusterid <- sample(1:50, NROW(lees), replace=TRUE)
    expect_message(s1h <- RDHonest(clusterid~margin, data=lees,
                                   se.method="EHW", h=5, kern="uniform"))
    expect_message(s1c <- RDHonest(clusterid~margin, data=lees,
                                   se.method="EHW", h=5,
                                   kern="uniform", clusterid=clusterid))
    ## Should equal
    ## m1 <- lm(clusterid~margin*I(margin>0), data=lees, subset=abs(margin)<=5)
    ## sqrt(sum(s1c$data$est_w[s1c$data$est_w!=0]^2*m1$residuals^2))
    ## or sqrt(sandwich::vcovHC(m1, type="HC0")[3, 3])
    expect_equal(as.numeric(s1h$coefficients[2:3]),
                 c(-10.83849263, 5.649144066))
    ## Should equal
    ## sandwich::vcovCL(m1, type="HC0", cluster=clusterid[abs(lee08$margin)<=5],
    ##                  cadjust=FALSE)[3, 3]
    expect_equal(as.numeric(s1c$coefficients[3])^2, 47.03214346)
    expect_message(s2c <- RDHonest(clusterid~margin, data=lees,
                                   se.method="EHW", clusterid=clusterid))
    h <- s2c$coefficients$bandwidth
    m2 <- lm(clusterid~margin*I(margin>0), data=lees,
             subset=abs(margin)<=h,
             weights = (1 - abs(margin/h)))
    ## sqrt(sandwich::vcovCL(m2, type="HC0",
    ##                       cluster=clusterid[abs(lees$margin)<=h],
    ##                       cadjust=FALSE)[3, 3])
    expect_equal(as.numeric(s2c$coefficients[2:3]),
                 c(unname(m2$coefficients[3]), 3.849691688))
