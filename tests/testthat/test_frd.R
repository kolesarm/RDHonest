...
test_that("Selected bw is infinite", {
    ...
    r0 <- NPRHonest(d, M=c(0, 0), kern="triangular", opt.criterion="OCI",
                    T0=0)$coefficients
    r1 <- NPRHonest(d, M=c(0, 0), kern="uniform", opt.criterion="FLCI",
                    T0=0)$coefficients
    r2 <- NPReg(d, Inf, "uniform")
    ...
    r1 <- NPRHonest(d, M=0, kern="uniform", opt.criterion="MSE")$coefficients
    r2 <- NPReg(d, Inf, "uniform")
    expect_equal(c(r1$std.error, r1$estimate),
                 c(r2$se, r2$estimate))
    expect_identical(r1$maximum.bias, 0)
...
test_that("FRD data example check", {
    ...
    M <- MROT(d)
    r1 <- NPRHonest(d, M, kern="triangular", opt.criterion="MSE",
                    T0=0)$coefficients
    r2 <- NPRHonest(d, M, kern="triangular", opt.criterion="MSE",
                    T0=r1$estimate)$coefficients
    r1a <- NPRHonest(d, M, kern="triangular", opt.criterion="MSE", T0=0,
                     alpha=r1$p.value)$coefficients
...
test_that("FRD with almost perfect first stage", {
    ...
    M <- MROT(d)
    r0 <- NPRHonest(d, M, kern="triangular", opt.criterion="FLCI")
    r1 <- NPRHonest(d, M, kern="triangular", opt.criterion="FLCI",
                    T0=r0$coefficients$estimate)$coefficients
    r2 <- NPRHonest(NPRData(lee08, cutoff=0, "SRD"), unname(M[1]),
    ...
    M <- MROT(d)
    r0 <- NPRHonest(d, M, kern="triangular", opt.criterion="MSE")
    r1 <- NPRHonest(d, M, kern="triangular", opt.criterion="MSE",
                    T0=r0$coefficients$estimate)$coefficients
    r2 <- NPRHonest(NPRData(lee08, cutoff=0, "SRD"), unname(M[1]),
...
test_that("FRD interface", {
    ...
    M <- MROT(d)
    rcp1 <- rcp[1:10000, ]
    r1 <- NPRHonest(d, M, kern="triangular", opt.criterion="OCI", T0=0)
    ...
    r2 <- NPRHonest(d, M, kern="triangular", opt.criterion="OCI",
    ...
    r4 <- OptBW(d, M, kern="triangular", opt.criterion="OCI",
                T0=r1$coefficients$estimate)
