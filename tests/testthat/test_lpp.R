...
test_that("Inference at point agrees with RD", {
    ...
    rde <- NPRHonest(d, h=5, M=2)$coefficients
    dp <- NPRData(lee08[lee08$margin>=0, ], cutoff=0, "IP")
    dm <- NPRData(lee08[lee08$margin<0, ], cutoff=0, "IP")
    p0 <- NPRHonest(dp, h=5, M=2)
    pp <- p0$coefficients
    mm <- NPRHonest(dm, h=5, M=2)$coefficients
...
test_that("Optimal bandwidth calculations", {
    ...
    r2 <- OptBW(dp, M=2*Mh, opt.criterion="MSE")
    ...
    r <- RDHonest(voteshare ~ margin, data=lee08, subset = (margin>0),
                  cutoff=20,
