...
package as data frames `rebp` and `headst`.
...
This approach gives tigher CIs than simply adding and subtracting
$B(\hat{\beta}_{h_+,h_-})$ from the point estimate, in addition to adding and subtracting $z_{1-\alpha}\hat{se}(\hat{\beta}_{h_+,h_-})$
...
tighter CIs. Comparing the lenghts of two-sided CIs with optimally chosen
bandwidths, using Silverman's rule of thumb to estimate the preliminary variance estimate used to compute optimal bandwidths:
...
$M$ for the Taylor and Hölder smoothess class, as described in the supplements to @KoRo16 and @ArKo16optimal
